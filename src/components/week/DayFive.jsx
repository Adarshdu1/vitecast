...
export default function DayFive({ weatherData, isCelcius }) {
  ...
  useEffect(() => {
    ...
    setId(weatherData.list[26].weather[0].id);
    if (isCelcius) {
      const tmp = weatherData.list[26].main.temp_min;
      setMinTemp(tmp.toFixed(0));
      const tmp2 = weatherData.list[26].main.temp_max;
