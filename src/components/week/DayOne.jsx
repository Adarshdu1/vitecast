...
export default function DayOne({ weatherData, isCelcius }) {
  const weekDays = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"];
  const [timezone, setTimezone] = useState(null);
  const date = new Date();
  const utc = date.getTime() + timezone * 1000 - 330 * 60000;
  const nd = new Date(utc);
  const day = weekDays[nd.getDay()];
  const [minTemp, setMinTemp] = useState(null);
  const [maxTemp, setMaxTemp] = useState(null);
  const [id, setId] = useState(null);
  useEffect(() => {
    setTimezone(weatherData.city.timezone);
    const idx = weatherData.list[0].weather[0].id;
    if (isCelcius) {
      const tmp = weatherData.list[0].main.temp_min;
      setMinTemp(tmp.toFixed(0));
      const tmp2 = weatherData.list[0].main.temp_max;
