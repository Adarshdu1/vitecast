...
export default function DaySix({ weatherData, isCelcius }) {
  ...
  useEffect(() => {
    ...
    setId(weatherData.list[32].weather[0].id);
    if (isCelcius) {
      const tmp = weatherData.list[32].main.temp_min;
      setMinTemp(tmp.toFixed(0));
      const tmp2 = weatherData.list[32].main.temp_max;
